...
const chunks = fs.readdirSync('dist/client/_next/static/chunks').map((name) => {
  const data = fs.readFileSync(
    path.join('dist/client/_next/static/chunks', name),
  );
...
console.log(
  JSON.stringify(
    {
      scope:
        'Built Worker on loopback; warm full-response latency, not network or real-device Core Web Vitals. Gzip is computed, not assumed transport compression.',
      chunks,
      routes,
    },
    null,
