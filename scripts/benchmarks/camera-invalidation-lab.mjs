...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST') {
      ...
      if (pathname !== '/results' && pathname !== '/progress' && !(experiment === 'shadow' && pathname === '/bake') && !(experiment === 'contact' && pathname === '/contact-bake') && !(experiment === 'diffuse' && pathname === '/diffuse-bake')) {
        json(response, 405, { error: 'This fixture has no application write APIs.' }); return;
      }
      const value = await body(request);
      if (pathname === '/contact-bake' || pathname === '/diffuse-bake') {
        const diffuse = pathname === '/diffuse-bake';
        const bakerHash = diffuse ? diffuseBakerHash : contactBakerHash;
        ...
        const descriptor = join(evidence, `bake-${hash(sourceHash + bakerHash).slice(0, 16)}.json`);
        try {
          const asset = JSON.parse(await fs.readFile(descriptor, 'utf8'));
          const gzip = await fs.readFile(join(evidence, asset.filename));
          if (hash(gzip) !== asset.gzipSha256 || asset.sourceHash !== sourceHash || asset.bakerSha256 !== bakerHash) throw new Error('Cached developer bake failed its hash check.');
          bakedAssets.set(asset.url, { gzip, metadata: asset });
          json(response, 200, { asset, reusedDeveloperBake: true }); return;
        } catch (error) { if (error.code !== 'ENOENT') throw error; }
        const payload = diffuse ? fitDiffuseProbe(value) : bakeContactGeometry(value);
        const raw = Buffer.from(JSON.stringify(payload));
        const gzip = gzipSync(raw, { level: 9 });
        const brotli = brotliCompressSync(raw);
        const filename = `${diffuse ? 'probe' : 'contact'}-${hash(raw).slice(0, 16)}.json.gz`;
        await fs.mkdir(evidence, { recursive: true });
        await fs.writeFile(join(evidence, filename), gzip, { flag: 'wx' }).catch((e) => { if (e.code !== 'EEXIST') throw e; });
        const asset = { filename, sourceHash, bakerSha256: bakerHash, sha256: hash(raw), gzipSha256: hash(gzip),
