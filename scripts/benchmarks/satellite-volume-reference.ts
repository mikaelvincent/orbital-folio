/** Frozen production environment before the 2K combined Earth texture change.
 * Captured 2026-09-14. Only relative imports differ from the captured source.
...
type EnvironmentOptions = {
  mobile?: boolean;
  /** Developer audits can inject the exact shipped field without network access. */
  cloudFieldData?: Uint8Array;
};

/** Ocean and a developer-baked cloud volume, driven by the caller active clock. */
export function createOrbitalEnvironment(
  ...
  let cloudTexture = createCloudTexture(
    THREE,
    new Uint8Array([0, 0, 128, 128]),
    1,
    1,
  );
  const cloudStatus = {
  ...
  const installCloudData = (data: Uint8Array) => {
    if (data.byteLength !== CLOUD_FIELD_WIDTH * CLOUD_FIELD_HEIGHT * 4)
      throw new Error('Invalid cloud field length');
    if (cloudDisposed) return;
    const nextTexture = createCloudTexture(
      THREE,
      data,
      CLOUD_FIELD_WIDTH,
      CLOUD_FIELD_HEIGHT,
    );
    cloudMaterial.uniforms.cloudField.value = nextTexture;
    cloudTexture.dispose();
    cloudTexture = nextTexture;
    cloudStatus.ready = true;
    cloudStatus.readyMs = performance.now() - generationStarted;
    invalidate();
  };
  const cloudReady = options.cloudFieldData
    ? Promise.resolve().then(() => {
        installCloudData(options.cloudFieldData!);
        cloudStatus.source = 'injected-baked-field';
      })
    : (async () => {
        try {
          const start = performance.now();
          const response = await fetch(CLOUD_ASSET, {
            signal: assetAbort.signal,
          });
          if (!response.ok)
            throw new Error(`Cloud asset HTTP ${response.status}`);
          const packed = await response.arrayBuffer();
          cloudStatus.fetchMs = performance.now() - start;
          const contentLength = response.headers.get('Content-Length');
          cloudStatus.encodedBytes = contentLength
            ? Number(contentLength)
            : null;
          cloudStatus.responseBytes = packed.byteLength;
          const decodeStart = performance.now();
          // Some hosts serve .gz with Content-Encoding:gzip, so fetch has already
          // decompressed it. Others serve the opaque archive; accept both paths.
          ...
          const packet =
            bytes[0] === 0x1f && bytes[1] === 0x8b
              ...
              : bytes;
          const field = decodeCloudField(packet);
          if (
            field.width !== CLOUD_FIELD_WIDTH ||
            field.height !== CLOUD_FIELD_HEIGHT
          )
            throw new Error('Unexpected cloud atlas dimensions');
          const data = field.data;
          cloudStatus.decodeMs = performance.now() - decodeStart;
          installCloudData(data);
          cloudStatus.source = 'developer-baked-atlas';
        } catch (error) {
          if (cloudDisposed) return;
          cloudStatus.error =
          ...
          cloudStatus.generationMs = performance.now() - start;
          installCloudData(field.data);
          cloudStatus.source = 'generated-recovery';
        }
      })();
  ...
  const cloudFieldBytes = CLOUD_FIELD_WIDTH * CLOUD_FIELD_HEIGHT * 4;
  const proceduralTextureBytes = cloudFieldBytes + skyData.byteLength;
  let cloudGpuBytes = 0;
  for (
    let w = CLOUD_FIELD_WIDTH, h = CLOUD_FIELD_HEIGHT;
    ;
    w = Math.max(1, w >> 1), h = Math.max(1, h >> 1)
  ) {
    cloudGpuBytes += w * h * 4;
  ...
  const proceduralTextureGpuBytes = cloudGpuBytes + skyGpuBytes;
  ...
  earth.add(surface);
  const cloudMaterial = createCloudVolume(THREE, cloudTexture);
  const clouds = new THREE.Mesh(sphereGeometry, cloudMaterial);
  clouds.scale.setScalar(CLOUD_TOP_RADIUS);
  ...
  return {
    ...
    update(time: number, moving: boolean, x: number, y: number) {
      ...
      cloudMaterial.uniforms.readyOpacity.value = cloudStatus.ready ? 1 : 0;
      cloudSunRotation
      ...
      cloudMaterial.uniforms.cameraLocal.value
        .copy(camera.position)
        .sub(earth.position)
        .applyQuaternion(cloudSunRotation)
        .divideScalar(CLOUD_TOP_RADIUS);
    ...
    getDiagnostics() {
      ...
      return {
        ...
        cloudFieldSamples: CLOUD_STEPS,
        cloudInterpolation: 'rgba8-gradient-mips-anisotropy-4',
        cloudWeatherModel: 'satellite-scattered-variable-depth',
        cloudLightingModel:
          'spherical-height-volume-optical-depth-baked-slopes',
        cloudSunProbes: 0,
        cloudMorph: [0, 0, 0],
        cloudReady: cloudStatus.ready,
        cloudSource: cloudStatus.source,
        cloudLoadError: cloudStatus.error,
        cloudFieldFetchMs: cloudStatus.fetchMs,
        cloudFieldDecodeMs: cloudStatus.decodeMs,
        cloudFieldGenerationMs: cloudStatus.generationMs,
        cloudFieldReadyMs: cloudStatus.readyMs,
        cloudEncodedBytes: cloudStatus.encodedBytes,
        cloudResponseBytes: cloudStatus.responseBytes,
        starCount: count,
        starBufferBytes:
        ...
        textureSize: CLOUD_FIELD_WIDTH,
        dayTextureSize: 0,
        cloudTextureSize: CLOUD_FIELD_WIDTH,
        proceduralTextureBytes,
        proceduralTextureGpuBytes,
        proceduralGenerationMs: Math.round(generationMs * 100) / 100,
        proceduralNoiseDimensions: [CLOUD_FIELD_WIDTH, CLOUD_FIELD_HEIGHT],
        proceduralNoiseFormat: 'RGBA8',
        proceduralNoiseMipBytes: cloudGpuBytes,
        proceduralNoiseMipLevels: Math.log2(CLOUD_FIELD_WIDTH) + 1,
        nebulaDimensions: [skyWidth, skyHeight],
        externalTextureRequests: options.cloudFieldData ? 0 : 1,
        estimatedTextureMiB:
