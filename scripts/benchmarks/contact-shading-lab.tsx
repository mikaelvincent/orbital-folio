/** Public-fixture comparison of cached live GTAO and offline surface AO.
...
const labels = { A: 'Delivered live GTAO', B: 'Baked static surfaces', C: 'Bake with live contact zones' };
const rooms = ['projects', 'about', 'experience', 'contact'];
const panel = document.getElementById('camera-lab-controls')!;
panel.innerHTML = `<style>#camera-lab-controls [data-running="true"] h2,#camera-lab-controls [data-running="true"] label,#camera-lab-controls [data-running="true"] details,#camera-lab-controls [data-running="true"] p:not(#contact-status),#camera-lab-controls [data-running="true"] .camera-lab-actions button:not(#contact-stop){display:none}</style><section class="camera-lab-panel"><h2>Contact shading · surface-bake lab</h2>
<p id="contact-status" role="status">Preparing the delivered runtime…</p>
<div class="camera-lab-actions"><button id="contact-baseline" disabled>1. Survey current rooms</button>
<button id="contact-verify" disabled>2. Bake and inspect</button><button id="contact-timing" disabled>3. Rested comparisons</button>
<button id="contact-survey" disabled>4. Unranked cost survey</button><button id="contact-stop">Stop</button></div>
<p>A keeps delivered GTAO. B bakes static Projects surfaces while other surfaces retain live shading. C also keeps live contact shading around doors and the reader. These are developer candidates, not portfolio settings.</p>
<label>Conditions <input id="contact-conditions" placeholder="Power source, other workload, placement"></label>
<details><summary>Results and limitations</summary><textarea id="contact-output" readonly aria-label="Contact-shading experiment results"></textarea></details></section>`;
const button = (name: string) => document.getElementById(`contact-${name}`) as HTMLButtonElement;
let controller: SceneAuditController;
let context: Parameters<NonNullable<SceneAudit['contactReady']>>[0];
let session: ReturnType<typeof createContactBakeSession>;
let asset: any;
...
const audit: SceneAudit = {
  manual: true,
  contactReady(value) { context = value; session = createContactBakeSession(value); return () => session.dispose(); },
  contactFrame() { session?.update(); },
  ready(value) {
...
async function save(report: any) {
  ...
  (document.getElementById('contact-output') as HTMLTextAreaElement).value = JSON.stringify({ status: report.status, saved,
...
async function baseline(report: any) {
  await status('Surveying all four rooms in forward/reverse order; descriptive pass costs only');
...
async function bake() {
  if (asset) return;
  await status('Exporting static surfaces for the developer bake');
  await settle('projects');
  const started = performance.now();
  const response = await fetch('/contact-bake', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(session.exportInput()) });
  if (!response.ok) throw new Error(await response.text());
...
async function verify(report: any) {
  await bake(); report.asset = asset; let images = 0;
  ...
  for (const room of ['experience', 'contact', 'home']) { await settle(room); await compare(`${room}-settled`, room === 'home'); }
...
async function timing(report: any, unranked: boolean) {
  ...
  report.asset = asset;
  if (innerWidth < 700) { report.status = 'inapplicable-candidates-fall-back-to-baseline'; return; }
  if (unranked) {
...
async function start(mode: Mode) {
  ...
  const report: any = { schemaVersion: 1, runId: `contact-${mode}-${innerWidth}x${innerHeight}-${Date.now()}`, mode, status: 'running',
    startedAt: new Date().toISOString(), settings: controller.snapshot().settings,
    conditions: (document.getElementById('contact-conditions') as HTMLInputElement).value,
    ...
    limitations: ['Developer public-seed fixture in the browser identified by settings.userAgent; not native Safari unless actually run there.',
      ...
      'A is delivered live GTAO. B loses moving contacts onto baked surfaces; C restores bounded live zones but may show zone seams or missing contacts outside them.',
      ...
      'No hidden quality changes: drawing buffer, view state and settings are checked. Below 700 CSS pixels the session may fall back to A.',
  ...
  try {
    ...
    catch (error) { document.getElementById('contact-status')!.textContent = `Result save failed: ${String(error)}`; }
