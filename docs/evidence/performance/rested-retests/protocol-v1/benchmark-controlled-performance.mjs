...
const sourcePaths = [
  ...
  'components/flush-window-reveals.ts',
  'components/case-study-archive.ts',
  'components/rounded-cabin-interior.ts',
  'lib/spacecraft-wall-layout.ts',
  'components/iris-hatch.ts',
...
function kernel(fixture, variant, iterations) {
  fixture.reset();
...
async function sample(fixture, variant, iterations, role) {
  ...
  const result = kernel(fixture, variant, iterations);
...
function calibrate(fixture) {
  if (fixture.kind === 'construction') {
    ...
    return {
      iterations: 1,
      reason:
        'one indivisible startup operation per sample; construction may exceed burst target',
    };
  }
  let iterations = 1;
  const probes = [];
