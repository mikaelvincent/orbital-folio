...
const root = resolve(process.argv[2] || process.cwd()),
  ...
  output = resolve(process.argv[4] || '/tmp/ladder-panel-lighting-critic.json');
const hash = () =>
...
const report = {
  sourceSha256: sha,
  scope:
    'Post-removal lighting preservation: selected-room doorway material invariance through animated stairs hover, leave, reversal, and destination portal hover. Cove rays target the surviving ladder rear mesh only, not full-scene visibility or closure. GPU and first-hit removal checks are separate.',
  cases: [],
  failures: [],
};
for (const layout of ['wide', 'compact'])
  for (const room of ['projects', 'about']) {
    ...
    for (const x of [0.4, 0.52, 0.57, 0.6, 0.64, 0.668])
      for (const dy of [-0.3, 0, 0.3]) {
        ...
        boundarySamples.push({ x, dy, expectedOwner: 'walkway', hit });
      }
    ...
    if (reveal.length !== 1 || rearReturn.length !== 0 || !ladder.length)
      c.failures.push(
        'Missing selected reveal or actual ladder, or obsolete rear-return mesh remains',
      );
    ...
    for (const stage of [
      ...
      for (let i = 0; i < stage.frames; i++) {
        ...
        model.update(
          time,
          stage.portal ? (room === 'projects' ? 'about' : 'projects') : '',
          false,
          {
            ...state,
            hoveredWalkway: stage.hover,
            hoveredPortal: stage.portal
              ? room === 'projects'
                ? 'about'
                : 'projects'
              : '',
