...
const report = JSON.parse(
  fs.readFileSync(
    `docs/evidence/portrait-continuous-flight/coverage-${mesh}.json`,
  ),
);
...
const samples = JSON.parse(
  gunzipSync(
    fs.readFileSync(
      `docs/evidence/portrait-continuous-flight/coverage-${mesh}-poses.json.gz`,
    ),
  ),
).samples;
...
const output = {
  ...
  assumptions: [
    `${mesh === 'mobile' ? 'Mobile96x64' : 'Desktop128x96'} sphere; orientation-resize cross-product samples whose expanded half-space bound exceeds the 64-row margin.`,
...
fs.writeFileSync(
  `docs/evidence/portrait-continuous-flight/coverage-perturbations-${mesh}.json`,
