...
const report = JSON.parse(
  fs.readFileSync(`docs/evidence/portrait-inward-flight/coverage-${mesh}.json`),
);
const replaySourceVerification = {};
for (const [path, expected] of Object.entries(report.sourceSha256)) {
  const actual = createHash('sha256')
    .update(fs.readFileSync(path))
    .digest('hex');
  const usedByReplay =
    path.startsWith('features/orbit/') ||
    path === 'scripts/benchmarks/mesh-uv-coverage.mjs';
  replaySourceVerification[path] = {
    expected,
    actual,
    matches: actual === expected,
    usedByReplay,
  };
  if (usedByReplay && actual !== expected)
    throw new Error(`Replay input changed: ${path}`);
}
const samples = JSON.parse(
  gunzipSync(
    fs.readFileSync(
      `docs/evidence/portrait-inward-flight/coverage-${mesh}-poses.json.gz`,
    ),
  ),
).samples;
const base = samples.filter(
  (s) =>
    s.guarded &&
    (s.guarded.sourcePixelRows[0] < 448 || s.guarded.sourcePixelRows[1] > 1856),
);
...
for (const sample of base) {
  ...
  if (sample.viewportCompositionRadians >= Math.PI / 2) {
    environment.setViewportComposition(true, reference, true);
  } else if (sample.viewportCompositionRadians > 0) {
...
const output = {
  ...
  replaySourceVerification,
  assumptions: [
    `${mesh === 'mobile' ? 'Mobile96x64' : 'Desktop128x96'} sphere; every sample whose expanded half-space bound exceeds the 64-row margin.`,
    'Each sample tests neutral plus 26 normalized [-1,0,1]^3 translation directions at radius 0.25 orbital units, crossed with neutral plus 26 local-camera rotation axes at 5.5 degrees: 729 exact frusta per case.',
    'This finite perturbation sampling is not an exhaustive optimization or mathematical proof for every point in the bounded neighborhood. Extreme drags are already at configured limits; additional rotations may exceed reachable production drag.',
    'Recorded camera transforms are replayed directly. Orbital and mesh-clipping sources must match the audit. Camera helper/runtime hashes may differ only as disclosed by replaySourceVerification: they are not executed during this recorded-camera follow-up.',
    'No texture decode, rendering or GPU timing.',
  ],
  cases,
};
fs.writeFileSync(
  `docs/evidence/portrait-inward-flight/coverage-perturbations-${mesh}.json`,
