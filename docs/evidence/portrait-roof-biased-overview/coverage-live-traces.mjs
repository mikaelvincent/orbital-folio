...
const traces = Object.fromEntries(
  ['entry-trace.json', 'return-trace.json', 'left-drag-trace.json', 'right-drag-trace.json'].map(
    (name) => {
...
for (const mobile of [false, true]) {
  ...
  for (const [name, trace] of Object.entries(traces)) {
    const [width, height] = name.includes('drag') ? [768, 1024] : [390, 844];
    environment.resize(width, height, 1, responsiveCameraFov(width / height));
  ...
  JSON.stringify(
    {
      ...
      assumptions: [
        'Final built-in Chromium traces: entry/return at actual 390x844, left/right drag at768x1024 CSS pixels, DPR1, with a settled portrait Earth anchor. Replay uses recorded background camera transforms directly.',
        'Entry and return are full Contact transitions. Drag traces retain their last250 frames including release. This is finite evidence for recorded frames; function tests exercise exact signed drag extrema. It does not cover every interruption or browser.',
